...
//rook checks
        ...
        if ((abs(whiteKingDown - blackKingDown) == 1 && abs(blackKingRight - whiteKingRight) == 1)
            || (abs(whiteKingDown - blackKingDown) == 0 && abs(blackKingRight - whiteKingRight) == 1)
            || (abs(whiteKingDown - blackKingDown) == 1 && abs(blackKingRight - whiteKingRight) == 0)) {
            chessboard[right][down] = piece;
            chessboard[dropRight][dropDown] = takenPiece;
            if (piece == "K") {
                whiteToMove = true; blackToMove = false;
        ...
        if (programMove == false) {
            ...
            if (blackKingStillInCheck && isWhiteKingInCheck){
                chessboard[right][down] = piece;
                chessboard[dropRight][dropDown] = takenPiece;
                whiteToMove = false;blackToMove = true;
                cout<<"You'll be in check if you play this!"<<endl;continue;
            }else if (isBlackKingInCheck && whiteKingStillInCheck){
                chessboard[right][down] = piece;
                chessboard[dropRight][dropDown] = takenPiece;
                whiteToMove = true;blackToMove = false;
                cout<<"You'll be in check if you play this!"<<endl;continue;
            ...
            if (blackToMove && isWhiteKingInCheck && !(piece == "K" && abs(down - dropDown) == 2)) {
                chessboard[right][down] = piece;
                chessboard[dropRight][dropDown] = takenPiece;
                whiteToMove = true;blackToMove = false;
                cout<<"You'll be in check if you play this!"<<endl;continue;
            }else if (whiteToMove && isBlackKingInCheck && !(piece == "k" && abs(down - dropDown) == 2)) {
                    chessboard[right][down] = piece;
                    chessboard[dropRight][dropDown] = takenPiece;
                    whiteToMove = false;blackToMove = true;
                    cout<<"You'll be in check if you play this!"<<endl;continue;
...
// if the passing square during a castle isn't attacked, castling is allowed otherwise the castle is declined
        ...
        if (programMove == false) {
            ...
            //if it enters here the king hasn't castled into a check and can proceed
